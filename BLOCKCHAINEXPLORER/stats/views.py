...
def stats_index(request):
    ...
    fig = px.line(df, x=x_data_EthPrice, y=y_data_EthPrice, labels = {'x' : "Day",'y' : "Price"}, title = "ETH 14D PRICE")
    plot_div = plot(fig,output_type = 'div')
    
    x_data_DailyTx = dict['x_data_EthTxCt']
    y_data_DailyTx = dict['y_data_EthTxCt']
    fig = px.line(df, x=x_data_DailyTx, y= y_data_DailyTx, labels = {'x' : "Day",'y' : "Amount of Transactions"} , title = "ETHEREUM TRANSACTION HISTORY IN 14 DAYS")
    plot_tx = plot(fig,output_type = 'div')
    
    x_data_TopWallet = dict['x_data_TopWallet']
    y_data_TopWallet = dict['y_data_TopWallet']
    fig = px.histogram(df, x=x_data_TopWallet,y=y_data_TopWallet,  labels = {'x' : "Wallet Address",'y' : "Amount in Eth"} , title = "TOP 10 RICHEST ADDRESS IN ETHEREUM")
    plot_TopWallet = plot(fig,output_type = 'div')

    x_data_PieMc = dict['x_data_PieMc']
    y_data_PieMc = dict['y_data_PieMc']
    fig = px.pie(df,values=x_data_PieMc, names=y_data_PieMc, title = "MARKETCAP REPARTITION")
